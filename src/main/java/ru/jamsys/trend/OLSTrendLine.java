...
public abstract class OLSTrendLine implements TrendLine {
    ...
    public void setValues(double[] y, double[] x) {
        ...
        coefficient = MatrixUtils.createColumnRealMatrix(ols.estimateRegressionParameters()); // get our coefs
    }

    @Override
    public double predict(double x) {
        double yHat = coefficient.preMultiply(xVector(x))[0]; // apply coefs to xVector
